...
void half_adder_test::stimulus(){
	...
	while(true){
		clr.write(rand() % 2);
		a.write(rand() % 2);      ///// 1 << data_width is nothing but: 1 * 2^data_width: will give me random valuw between 0 to 2^datawidth-1 
		b.write(rand() % 2);       ///////// if data _width is 4 than : randomize between 0 to 2^4-1 i.e 0 to 15
